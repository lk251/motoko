...
def parse_python_symbols(path: pathlib.Path, root: pathlib.Path) -> tuple[list[CodeSymbol], list[dict], list[dict], list[dict]]:
    ...
    Visitor().visit(tree)
    return symbols, imports, commands, []
...
def build_code_map(root: str | pathlib.Path | None = None) -> dict:
    ...
    for path in files:
        text = _safe_read(path)
        file_symbols, file_imports, file_commands, file_errors = parse_python_symbols(path, repo_root)
...
def format_code_map_report(code_map: dict) -> str:
    ...
    lines = [
        ...
        f"commands: {summary.get('commands', 0)}  handlers: {summary.get('command_handlers', 0)}  tests: {summary.get('tests', 0)}",
...
def _format_rows(title: str, rows: list[dict], *, command: bool = False) -> list[str]:
    lines = [title + ":"]
    if not rows:
        lines.append("- none")
        return lines
